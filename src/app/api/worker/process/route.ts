...
export async function POST(req: Request) {
  ...
  void jobId; // the Job is addressed via its unique statementId; jobId is carried for tracing

  // (1c) load the Statement. Not found => 200 (deleted; don't make QStash retry forever).
  // There is NO user session here (machine-to-machine); the QStash signature is the auth, and we
  // operate only on the verified statementId. userId lives on the row itself, not a session.
